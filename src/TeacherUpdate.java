...
public class TeacherUpdate extends JFrame implements ActionListener{
    ...
    TeacherUpdate() {
        ...
        lblaadhar.setBounds(400, 260, 200, 30);
        lblaadhar.setFont(new Font("serif", Font.BOLD, 20));
        add(lblaadhar);

        JLabel cnic = new JLabel();
        cnic.setBounds(600, 260, 150, 30);
        cnic.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(cnic);
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == submit) {
            ...
            if(!check.validated(address, phone,email)){
                return;
            }else{
                try {
                    String query = "update teacher_info set address='"+address+"', phone='"+phone+"', email='"+email+"', qualification='"+course+"' where emp_id='"+empId+"'";
                    ConnectToMySql con = new ConnectToMySql();
                    con.s.executeUpdate(query);

                    JOptionPane.showMessageDialog(null, "Teacher Details Updated Successfully");
                } catch (Exception e) {
                    JOptionPane.showMessageDialog(null, "An error occurred please check the errors returned by stack trace!");
                    e.printStackTrace();
                }
            }
        } else {
